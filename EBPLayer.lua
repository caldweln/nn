...
function EBPLayer:__init(inputSize, outputSize, opt)
  ...
  if opt.arch.sigma and opt.arch.sigma > 0 then
    self:reset(1,opt.arch.sigma)  -- original method
  elseif opt.arch.alpha and opt.arch.alpha > 0 then
    self:reset(opt.arch.alpha) -- sigmoid or other methods
  else
